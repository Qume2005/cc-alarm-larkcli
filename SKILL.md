...
version: 1.0.0
description: "用 lark-cli 给你发飞书消息：任务完成、需要确认、长任务进度、出错告警。当 agent 跑完任务、卡在决策、跑训练/编译等长任务要周期 ping、或任务失败时调用 notify.sh 推一条飞书通知。轻量、非致命、节流友好。现已为插件，含自动 hook：Stop 推任务完成、权限请求 Notification 推需要确认（空闲等待与子 agent 完成已抑制降噪）。"
metadata:
  requires:
    bins: ["lark-cli"]
...
本技能现在是一个 **`@skills-dir` 插件**（`.claude-plugin/plugin.json` + 软链 `~/.claude/skills/cc-alarm-larkcli`），Claude Code 启动时自动发现加载，**零安装、无需 marketplace**。插件带 3 个自动 hook：在生命周期事件触发时**直接调用 `notify.sh` 发飞书**，不依赖模型记得用 skill——agent 停下或等你授权，就自动推送。
...
| `Notification`（权限请求） | agent 等你授权某操作 | `ask` 🙋 | hook stdin 的 message + 从 transcript 最近一个 `tool_use` 补充「工具：名称 + 关键细节」 |
| `Notification`（空闲等待） | "Claude is waiting for your input" | **不推送**（抑制） | — |

**SubagentStop 抑制**：子 agent 完成不再单独 ping——主 agent 的 `Stop` 会携带本轮汇总，每个子 agent 再 ping 一次纯属噪音。

**Notification 分两种**：
- **空闲等待**（"waiting for your input" 这类）→ **抑制不发**（agent 自动跑流程里这是误报）。
- **权限请求**（"needs your permission…"）→ **发，并补充细节**：除转发原始 message，尽力（best-effort）读 transcript 最近一个 `tool_use` 块——`Bash`→工具名+命令（截 150），`Edit`/`Write`/`Read`/`NotebookEdit`/`MultiEdit`→工具名+文件路径，其他→工具名+截断 JSON。读不到则静默回退到只发原始 message，不丢通知、不崩。

**节流（脚本内，防刷屏）**：`done` 默认 120s、`ask` 默认 60s；在 `~/.config/cc-alarm-larkcli/config.sh` 用 `HOOK_THROTTLE_DONE` / `HOOK_THROTTLE_ASK`（秒数）覆盖。`SubagentStop` 抑制后，`done` 节流只对 `Stop` 触发的推送生效；`ask` 节流只对权限请求类 `Notification` 生效（空闲类已被抑制）。`progress` 仍用 `THROTTLE_SECONDS`（手动场景）。
...
**默认按 markdown 发送**（lark-cli `--markdown`，飞书 post 富文本）——`**bold**`、列表、`##` 标题正常渲染，不再出现字面 `**`。固定头 `## <emoji> <标题>\n\n` 自动前置、**不可配置**；你的消息若也以 `#` 开头会出现两个相邻标题（预期 UX）。想发不渲染的纯文本，用 `--text`/`--plain` 强制切回（固定头退化为 `emoji 标题\n`，正文原样拼接）。

| Flag | 作用 | 备注 |
|------|------|------|
| `--markdown` | （默认）按 markdown 发（lark-cli 自动包成 post 富文本） | 不加 flag 也是 markdown，显式传入仅为声明，保留是为向后兼容。 |
| `--text` / `--plain` | 强制纯文本 `--text` 模式，不渲染 markdown | 用这个 opt out 默认 markdown。纯文本模式正文原样拼接，不做归一化。 |
